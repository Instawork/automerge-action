...
async function update(context, dir, url, pullRequest) {
  ...
  let action = null;
  logger.info("pullRequest :"+JSON.stringify(pullRequest));
  for (const label of pullRequest.labels) {
    logger.info("labels :"+JSON.stringify(label));
    if (actions.includes(label.name)) {
      logger.info("labels name :"+label.name);
      if (action === null) {
        logger.info("action null :"+label);
        action = label.name;
      } else {
